...
export default function WhatsAppInbox({ companyId, userId, userName, userRole, operatorRoles, initialConversationId, onConversationOpened }: WhatsAppInboxProps) {
  ...
  useEffect(() => {
    ...
    // Reset unread_count as requested
  ...
  const loadMessages = async (convId: string) => {
    ...
    console.log("[loadMessages] outbound:", data?.filter(m => m.direction === 'outbound').length);
            ...
            <div className="p-2 md:px-6 py-2 border-t border-border/5 bg-background/50 backdrop-blur-xl z-20 sticky bottom-0">
              ...
              {selectedConv && (
                puedeEnviar ? (
                  ...
                  <div className="max-w-4xl mx-auto bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 rounded-xl p-3 text-center text-[11px] mb-2 leading-snug font-medium">
                    ⚠️ No puedes enviar mensajes. Han pasado más de 24 horas desde el último mensaje del cliente. Solo puedes responder dentro de las 24 horas posteriores al último mensaje recibido. Para contactar al cliente debes usar una plantilla aprobada.
                  </div>
                )
              )}
